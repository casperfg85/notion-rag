...
class NotionAPIClient:
    ...
    def _retry_with_backoff(self, func, *args, **kwargs):
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                wait_time = self.config.api_delay * (self.config.backoff_factor ** attempt)
                
                log_with_extra(
                    self.logger, 'WARNING', 'API call failed, retrying',
                    function=func.__name__,
                    attempt=attempt + 1,
                    error=str(e),
                    wait_time=wait_time
                ...
                if attempt == self.config.max_retries - 1:
                    log_with_extra(
                        self.logger, 'ERROR', 'API call failed permanently',
                        function=func.__name__,
                        error=str(e)
                    )
                    raise
    ...
    def query_database(self, database_id: str, start_cursor: Optional[str] = None) -> Dict[str, Any]:
