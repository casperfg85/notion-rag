...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create vector index from parsed Notion data"
    )
    parser.add_argument(
        "--root_entity_id", required=True, help="Root entity ID to create index for"
    )
    ...
    if not parsed_file.exists():
        print(
            f"Error: Parsed data not found at {parsed_file}. Run scripts/parse.py --root_entity_id {args.root_entity_id} first."
        )
        sys.exit(1)
    ...
    try:
        ...
        if indexer.get_table() is not None:
            response = (
                input("Vector index already exists. Recreate? (y/n): ").strip().lower()
            )
            recreate = response == "y"
